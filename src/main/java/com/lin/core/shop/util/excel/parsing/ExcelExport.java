...
import com.lin.core.shop.util.excel.ExcelDefinitionReader;
import com.lin.core.shop.util.excel.ReflectUtil;
import com.lin.core.shop.util.excel.config.ExcelDefinition;
import com.lin.core.shop.util.excel.config.FieldValue;
import com.lin.core.shop.util.excel.exception.ExcelException;
...
public class ExcelExport extends AbstractExcelResolver{
	 ...
	 * @throws Exception
	 */
	public ExcelExportResult createMultiExcel(Map<String, List> map, ExcelHeader header, List<String> fields) throws Exception{
		ExcelExportResult exportResult = null;
		...
		if(keys != null && keys.size() > 0){
			for(String key:keys){
				if(CollectionUtils.isNotEmpty(map.get(key))){
					//从注册信息中获取Bean信息
					ExcelDefinition excelDefinition = definitionReader.getRegistry().get(key);
					if(excelDefinition==null){
						throw new ExcelException("没有找到 ["+key+"] 的配置信息");
					}
					Class<?> realClass = map.get(key).get(0).getClass();
					if(realClass==excelDefinition.getClazz() || TypeUtils.isAssignable(excelDefinition.getClazz(),realClass)){
						excelDefinition = dynamicCreateExcelDefinition(excelDefinition,fields);
					}
					else if(TypeUtils.isAssignable(realClass,excelDefinition.getClazz())){
						...
						if(superClass!=Object.class){
							...
							throw new ExcelException("传入的参数类型是:"+map.get(key).get(0).getClass().getName()
									+"但是 配置文件的类型是: "+excelDefinition.getClazz().getName()+",参数既不是父类,也不是其相同父类下的子类,无法完成转换");
						}

					}
					if(null == workbook){
						exportResult = doCreateExcel(excelDefinition,map.get(key),header);
						workbook=exportResult.build();
					}else {
						exportResult = appendSheet(excelDefinition, map.get(key),header, workbook);
					}
				}else {
					ExcelDefinition excelDefinition = definitionReader.getRegistry().get(key);
					exportResult = doCreateExcel(excelDefinition,map.get(key),header);
	 ...
	 * @throws Exception
	 */
	protected ExcelExportResult appendSheet(ExcelDefinition excelDefinition, List<?> beans,ExcelHeader header,Workbook workbook) throws Exception {
		Sheet sheet = null;
		...
		System.out.println(excelDefinition);
		return exportResult;
	}
